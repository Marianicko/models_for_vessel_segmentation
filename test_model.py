...
if __name__ == "__main__":   
    ...
    script_dir = os.path.dirname(os.path.abspath(__file__))
    model_path = os.path.join(script_dir, 'models', 'model_unet_seg.tr')
    torch.save(st, model_path)
    print("Модель сохраняется в:", os.path.abspath('model_unet_RGB.tr'))

'''
img = Image.open(r"vessel_1.jpg").convert('RGB')
img = tr_img(img).unsqueeze(0)

model.eval()  # !!! ВАЖНО: переключаем в режим оценки !!!
with torch.no_grad():  # !!! ВАЖНО: отключаем вычисление градиентов !!!
    p = model(img).squeeze(0)
   
#p = model(img).squeeze(0) # здесь получаются логиты, на следующей строке преобразуем их в вероятности
x = nn.functional.sigmoid(p.permute(1, 2, 0))   #permute - перестановка каналов - PyTorch работает с тензорами в формате (C, H, W), а Matplotlib ожидает изображения в формате (H, W, C) или (H, W) для grayscale
x = x.detach().numpy() * 255    # преобразуем в [0, 255]

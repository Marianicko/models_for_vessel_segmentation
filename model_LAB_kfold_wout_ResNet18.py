...
def plot_training_history(history, fold_num):
    ...
    # Совмещённый график всех метрик (С ИСПРАВЛЕННОЙ ЛЕГЕНДОЙ)
    ...
    axes[1, 2].legend(loc='lower right', fontsize='small')  # ИЗМЕНЕНО: теперь легенда внизу справа
...
if __name__ == "__main__":
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(active_triplets)):
        ...
        # Модель и оптимизация (ИЗМЕНЕНО: используем UNetModel без pretrained энкодера)
        model = UNetModel(in_channels=3, num_classes=1)  # <-- обучение с нуля!
        ...
        for epoch in range(EPOCHS):
            ...
            if val_dice > best_val_dice + MIN_DELTA:
                ...
                torch.save(model.state_dict(), f'models/model_fold{fold+1}_from_scratch_{RANDOM_STATE}.pth')
        ...
        torch.save(best_model_state, f'models/model_fold{fold+1}_from_scratch_{RANDOM_STATE}_{timestamp}.pth')
    ...
    for fold, (model, best_state, val_loader) in enumerate(best_models):
        ...
        visualize_predictions_with_errors(
            ...
            device=device, 
            num_samples=2,
            save_path=f'predictions_fold{fold+1}_from_scratch.png'
        )
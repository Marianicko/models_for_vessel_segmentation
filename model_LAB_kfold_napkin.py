...
# Добавьте эти функции перед main, если их нет:
...
def dice_coefficient_masked(probs, vessel_targets, skin_masks, smooth=1e-6):
    ...
    probs: предсказанные вероятности (после sigmoid)
    vessel_targets: истинные маски сосудов
    skin_masks: маски кожи (1 - кожа, 0 - не кожа)
    """
...
if __name__ == "__main__":
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(active_triplets)):
        ...
        # Датасеты и загрузчики (как раньше)
        ...
        torch.save(best_model, f'models/model_fold{fold+1}_pretrained_{RANDOM_STATE}_{timestamp}.pth')

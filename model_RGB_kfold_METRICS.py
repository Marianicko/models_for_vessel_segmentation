...
class SegmentDataset(data.Dataset):
    ...
    def __getitem__(self, item):
        '''
        Открывает картинки по известному пути, 
...
if __name__ == "__main__":
    ...
    # Для хранения результатов (ОБНОВЛЕНО)
    fold_results = {
        'dice': [], 
        'iou': [], 
        'precision': [], 
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(active_triplets)):
        ...
        # Для хранения истории эпох (ОБНОВЛЕНО)
        history = {
            'epoch': [],
        ...
        for epoch in range(EPOCHS):
            ...
            if val_dice > best_val_dice + MIN_DELTA:
                ...
                torch.save(best_model_state, f'models/model_fold{fold+1}_pretrained_{RANDOM_STATE}.pth')
        ...
        if best_model_state is not None:
            ...
            # Для визуализации создаём отдельную модель
            best_model_for_viz = UNetWithPretrainedEncoder(in_channels=3, num_classes=1, pretrained=True)
            best_model_for_viz.load_state_dict(best_model_state)
            best_model_for_viz = best_model_for_viz.to(device)
            best_models.append((best_model_for_viz, val_loader))
    ...
    for fold, (model, val_loader) in enumerate(best_models):
        print(f"\nФолд {fold+1}:")
        model.eval()
